...
UI.Input.prototype.setType= function ( value ) {
...
UI.Input.prototype.disabled = function(inputvalue){
	var range=this.dom;
	range.disabled = 'disabled';
	var percent=Math.ceil(((range.value - range.min||0) / (range.max - range.min||0)) * 100);
	range.style.background='-webkit-linear-gradient(left, #5f5f5f 0%,#ababab ' + percent + '%, #1e1f22 ' + percent + '%)';
	if(inputvalue)
		inputvalue.style.color="#6d6b6b";
	return this;
};
UI.Input.prototype.deldisabled = function(inputvalue){
	var range=this.dom;
	range.disabled = '';
	var percent=Math.ceil(((range.value - range.min||0) / (range.max - range.min||0)) * 100);
	range.style.background='-webkit-linear-gradient(left, #6f931e 0%,#c1f257 ' + percent + '%, #1e1f22 ' + percent + '%)';
	if(inputvalue)
		inputvalue.style.color="#6f931e";
	return this;
};
//==========================================================
...
UI.createDiv=function(className,parent,content,type){
	...
	if(content){
		if(type=="i"||type=="t"){
			obj.dom.placeholder=content;
			obj.dom.value=null;
		}
		else
		obj.setTextContent(content);
	}
	if(parent.dom){
		parent.add(obj);
	}else{
		parent.appendChild(obj.dom);
	}
...
UI.Iframe.prototype.constructor = UI.Iframe;
